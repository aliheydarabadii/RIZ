%% Beam book for RIS configs with fixed TX.
clc, clear all
...
theta_sweep = deg2rad(-60:10:60);
phi_sweep   = deg2rad(-15:15:15);
...
for i = 1:numel(phi_sweep)
    for j = 1:numel(theta_sweep)
        ...
        vTrue   = exp(-1j*2*pi/lambda *( (M*RIS.dy*sin(tx.theta)*cos(tx.phi) + N*RIS.dz*sin(tx.phi)) ...
                                      +  (M*RIS.dy*sin(THETA)*cos(PHI)       + N*RIS.dz*sin(PHI)) ));             % RIS phase shift 
...
% Save to CSV file
writetable(cell2table(data), 'Beam_book_0.csv', 'WriteVariableNames', false);
